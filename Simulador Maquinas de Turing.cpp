 ...
 main ()
{
	...
	printf(" --> Ingrese el numero de estados que tiene la maquina: ");
	scanf("%d",&Q);
	...
	scanf("%d", &numF);
	int eF[numF];
	for (i=0;i<numF;i++) //Ciclo que captura  cada uno de los estados finales
    {
	...
	for (i=0;i<=Q-1;i++) //Ciclo que captura las transciones de estado a estado
     {
    	...
    	{
        	printf("\n\n--> El estado q%d con \"%c\" avanza a la derecha al estado q", i,sigma[j]);
			scanf("%d",&delta[i][j]);
     	}	
     }
     
	printf("\n\n--------------Maquina de Turing Cargada Correctamente---------------- \n");
		...
		for (i=0; i<numF; i++)
		   { 
			   if(edoActual==eF[i]) //Condición que compara el último estado con la última posicion del último simbolo
			   {
		     	...
		     	if(edoActual!=eF[i]) //Si la posición del último simbolo NO es igual a la de un estado final, imprimir rechazado
		    	{
